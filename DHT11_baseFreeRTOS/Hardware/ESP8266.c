...
void ESP8266_Restart(void)
{
    ...
    MyDelay_xms(1000);
    SerialLog_SendStrData("ESP8266: restart ===> ......OK!\n");
}
/*
**********************************************************
...
uint8_t ESP8266_ConnectWiFi(void)
{
    if(ESP8266_WiFiConnected() == CONNECTED) {
        SerialLog_SendStrData("ESP8266: wifi connected!\n");
        return TRUE;
    }else {
        SerialLog_SendStrData("ESP8266: connect wifi ===> ");
        ...
        if(receive_flag == 1){
            ...
            if(ESP8266_HasTargetInfo("WIFI CONNECTED")){
                SerialLog_SendStrData("......OK!\n");
                return TRUE;
            }else {
                SerialLog_SendStrData("......FAIL!Start reconnecting......\n");
        ...
        if(receive_flag == 1){
            ...
            if(ESP8266_ACK() == ACK_SUCCESS) {
                if(ESP8266_HasTargetInfo("+MQTTCONN:0,0,0,")){
                    SerialLog_SendStrData("ESP8266: MQTT not connected!\n");
                    return DISCONNECT;
                }else {
                    SerialLog_SendStrData("ESP8266: MQTT connected!\n");
                    return CONNECTED;
                }
            }else {
                SerialLog_SendStrData("ESP8266: MQTT query --- error!\n");
                return DISCONNECT;
            }
        }
    }
...
void ESP8266_ConnectMQTT(void)
{
    while(ESP8266_MQTTConnected() == DISCONNECT){
        SerialLog_SendStrData("ESP8266: MQTT reconnecting......!\n");
...
{
    if(ESP8266_ATIsOK()){
        SerialLog_SendStrData("ESP8266: AT ===> OK!\n");
        /* 重启ESP8266 */
        ESP8266_Restart();
        /* 连接WiFi */
        ESP8266_ConnectWiFi();
